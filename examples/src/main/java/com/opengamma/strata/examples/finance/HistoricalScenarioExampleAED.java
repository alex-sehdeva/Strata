 ...
 */
public class HistoricalScenarioExampleAED {
  ...
  private static void calculate(CalculationRunner runner) {
    ...
    CalculationRules rules = CalculationRules.of(functions, marketDataBuilder.ratesLookup(LocalDate.of(2016, 6, 8)));
    ...
    LocalDate valuationDate = LocalDate.of(2016, 6, 8);
  ...
  private static ScenarioDefinition buildHistoricalScenarios(
    ...
    List<Curve> usdDiscountCurves = scenarioDates.stream()
        ...
        .map(group -> group.findDiscountCurve(Currency.AED).get())
        .collect(toImmutableList());

    List<Curve> libor3mCurves = scenarioDates.stream()
        .map(date -> historicalCurves.get(date))
        .map(group -> group.findForwardCurve(IborIndices.AED_EIBOR_3M).get())
        .collect(toImmutableList());
    ...
    PerturbationMapping<Curve> discountCurveMappings = PerturbationMapping.of(
        Curve.class,
        MarketDataFilter.ofName(CurveName.of("AED-3ME")),
